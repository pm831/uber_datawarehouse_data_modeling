...
JOIN passenger_count_dim p  
  ON p.passenger_count_id=f.passenger_count_id  
JOIN trip_distance_dim t  
  ON t.trip_distance_id=f.trip_distance_id  
JOIN rate_code_dim ra 
  ON ra.rate_code_id=f.rate_code_id  
JOIN pickup_location_dim pick 
  ON pick.pickup_location_id=f.pickup_location_id
JOIN dropoff_location_dim dropo 
  ON dropo.dropoff_location_id=f.dropoff_location_id
JOIN payment_type_dim pay 
